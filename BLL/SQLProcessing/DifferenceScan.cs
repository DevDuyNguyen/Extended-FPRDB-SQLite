...
{
    public class DifferenceScan:Scan
    {
        private Scan s1;
        private Scan s2;
        ...
        public DifferenceScan(Scan s1, Scan s2, ProbabilisticCombinationStrategy probCombinationStrategy, FPRDBSchema schema)
        {
            this.s1 = s1;
            this.s2 = s2;
        ...
        public bool next()
        {
            ...
            while (s1.next())
            {
                ...
                while (s2.next())
                {
                    ...
                    {
                        isMatched = true;
                        //differnce on t1 and t2 to produce the next tuple for the intersection
                        List<AbstractFuzzyProbabilisticValue> ans = this.differenceOnTuples();
        ...
        private List<AbstractFuzzyProbabilisticValue> differenceOnTuples()
        {
            FieldType fieldType;
            ...
            foreach (Field field in this.schema.getFields())
            {
                ...
                else
                {
                    ...
                    if (fieldType == FieldType.INT || fieldType == FieldType.DIST_FUZZYSET_INT)
                    {
                        ...
                        ans.Add(FProbValueCombinationStategy.difference<int>(fprobValue1, fprobValue2, this.probCombinationStrategy));
                    }
                    else if (fieldType == FieldType.FLOAT || fieldType == FieldType.DIST_FUZZYSET_FLOAT || fieldType == FieldType.CONT_FUZZYSET)
                    {
                        ...
                        ans.Add(FProbValueCombinationStategy.difference<float>(fprobValue1, fprobValue2, this.probCombinationStrategy));
                    }
                    else if (fieldType == FieldType.CHAR || fieldType == FieldType.VARCHAR || fieldType == FieldType.DIST_FUZZYSET_TEXT)
                    {
                        ...
                        ans.Add(FProbValueCombinationStategy.difference<string>(fprobValue1, fprobValue2, this.probCombinationStrategy));
                    }
                    else //if (fieldType == FieldType.BOOLEAN)
                    {
                        ...
                        ans.Add(FProbValueCombinationStategy.difference<bool>(fprobValue1, fprobValue2, this.probCombinationStrategy));
                    }
                }
            }
            return ans;
